function spriteSheetMaker(thisObj) {
    ...
    function buildSpriteSheet(cols) {
        try{
        var proj = app.project;
        var comp = proj.activeItem;
        if (!(comp instanceof CompItem)) return alert("Please open or select a comp before running this script.");
            
        app.beginUndoGroup("Create spritesheet");
        var lyrCount = 0;
        var dur = comp.duration/comp.frameDuration;
        var w = comp.width;
        var h = comp.height;
        ...
        for (var i = 0; i< ssComp.layers.length; i++) {
            var lyr = ssComp.layers[i+1];
            var wDriver = (i%cols)*w+ (w*.5);
            var hDriver = Math.floor (i/cols)*h+(h*.5);
            lyr.transform.position.setValue([wDriver, hDriver]);
            lyr.timeRemapEnabled = 1;
            lyr.timeRemap.removeKey(2);
            lyr.timeRemap.setValueAtTime(0,startFrame+(i*comp.frameDuration));
        }
    ...
    function buildUI(thisObj) {
        try{
        var pal = (thisObj instanceof Panel) ? thisObj : new Window("palette", scriptName, [200, 200, 600, 500], {resizeable: true});
        var ui = "group{orientation:'column', alignment:['fill','fill'],spacing:10,\
            logoGroup: Group{alignment:['fill','top'],preferredSize:[200,17],\
                ...
                versionNumber: StaticText {preferredSize:[20,8], alignment:['right','bottom']},\
            },\
            inputGroup: Group{orientation:'row',alignment:['fill','top'],alignChildren:['fill','bottom'],\
                columnsText: StaticText {minimumSize:[110,20], alignment:['left','bottom']},\
                columnsEditText: EditText{preferredSize:[30,20], alignment:['left','bottom']},\
            },\
            buttonGroup:Group{orientation:'column',alignment:['fill','top'],alignChildren:['fill','top'],\
                spriteSheetBtn: Button{preferredSize:[100,-1]},\
            }\
        }"; 
        pal.grp = pal.add(ui);
        ...
        // version number
        pal.grp.logoGroup.versionNumber.text = "v"+scriptVersion;
        
        pal.grp.inputGroup.columnsText.text = "Number of columns:";
        pal.grp.inputGroup.columnsEditText.text = 10;
        pal.grp.buttonGroup.spriteSheetBtn.text = "Build Sprite Sheet";
        pal.grp.buttonGroup.spriteSheetBtn.onClick = function() {buildSpriteSheet(pal.grp.inputGroup.columnsEditText.text)};
