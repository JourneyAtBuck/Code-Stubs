...
function bezierWarp2Nulls_AE() {
    try{
    
        app.beginUndoGroup("Un-bezierwarpify Selected Layers");
        ...
        for (var i=0; i<sel.length; i++) {
            ...
            if (sel[i].property("ADBE Effect Parade").property("ADBE BEZMESH")) {
                var slider = sel[i].Effects.addProperty("ADBE Slider Control");
                slider.name = "Offset Amount";
                var startIndex = 0;
                if (sel[i].hasTrackMatte) startIndex--;
                var cp = sel[i].property("ADBE Effect Parade").property("ADBE BEZMESH");
                for (p = 1; p<cp.numProperties-1; p++)  {
                ...
                for (c = 1; c<cp.numProperties-1; c++)  {
cp.property(c).expression = "var numVerts = 12;\r\
var total = [0,0];\r\
var center = [0,0];\r\
...
for (var v = 1; v<=numVerts; v++) {\r\
    ...
    var nullLyr = thisComp.layer(name+'_'+this.effect('ADBE BEZMESH')('ADBE BEZMESH-00'+propNum.toString()).name);\r\
