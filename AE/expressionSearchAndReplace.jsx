(function expressionSearchAndReplace(thisObj) {
    function searchAndReplace(oldText, newText) {
        var comp = app.project.activeItem;
        count = 0;
        if (comp != null && comp instanceof CompItem) {
            app.beginUndoGroup("Search and replace text in expressions."); //begins undo group
            (function recurseLayers (comp) {
                for (var c = 1; c <= comp.numLayers; c++) {
                    var lyr = comp.layer(c);
                    if (lyr.source instanceof CompItem) {
                        recurseLayers(lyr.source);
                    }
                    (function recurseProps (obj) {
                        for (var p = 1 ; p <= obj.numProperties; p++) {
                            if (obj.numProperties) recurseProps (obj(p));
                            var exp = obj(p).expression;
                            if (exp && exp.match(oldText)) {
                                obj(p).expression = exp.replace(oldText, newText);
                                count++;
                            }
                        }
                    })(comp.layer(c));
            ...
            app.endUndoGroup();
            return alert("Replaced expression "+count.toString()+" times.");
        } else return alert("Make sure your comp window or timeline is active before running this script.");
    }
    function buildUI(thisObj) {
        try{
        var pal = (thisObj instanceof Panel) ? thisObj : new Window("palette", "Expression search and replace", [200, 200, 600, 500], {resizeable: true});
        var ui = "group{orientation:'column', alignment:['fill','fill'],spacing:10,\
            ...
            mainGroup:Group{orientation:'column',alignment:['fill','top'],alignChildren:['fill','top'],\
                mainText: StaticText {minimumSize:[500,50],alignment:['middle','top']},\
                replaceGrp:Group{orientation:'row',alignment:['fill','top'],alignChildren:['fill','top'],\
                    oldTextLabel:StaticText{minimumSize:[70,25],alignment:['middle','top']},\
                    oldText:EditText{minimumSize:[180,25],alignment:['middle','top']},\
                    newTextLabel:StaticText{minimumSize:[70,25],alignment:['middle','top']},\
                    newText:EditText{minimumSize:[180,25],alignment:['middle','top']}\
                },\
        ...
        pal.grp.mainGroup.mainText.text = "Finds text in expressions within comps and nested precomps, replaces it";
        
        pal.grp.mainGroup.replaceGrp.oldTextLabel.text = "Search for:";
        pal.grp.mainGroup.replaceGrp.newTextLabel.text = "Replace with:";
        
        pal.grp.mainGroup.replaceBtn.text = "Search and Replace";
        pal.grp.mainGroup.replaceBtn.onClick = function() {searchAndReplace(pal.grp.mainGroup.replaceGrp.oldText.text, pal.grp.mainGroup.replaceGrp.newText.text)};
