(function makeCircle(thisObj) {
    ...
    function transferKeyframes(propertyInput, keysAry){
     ...
     // function returns size as [x,y], requires layer object
    ...
    function makeCircleShape (lyr) {
        
        var comp = app.project.activeItem;
        var inTrans = lyr("ADBE Transform Group");
        var transA = ["ADBE Anchor Point","ADBE Position","ADBE Scale","ADBE Rotate Z","ADBE Opacity"];

        var sLayer = comp.layers.addShape();
        var outTrans = sLayer("ADBE Transform Group");
        var contents = sLayer("ADBE Root Vectors Group");
        var circle = contents.addProperty("ADBE Vector Shape - Ellipse");
        var newSize = findSize(lyr);
        circle("ADBE Vector Ellipse Size").setValue(newSize);
        circle("ADBE Vector Ellipse Position").setValue(newSize*.5);
        
        circle = null;
        contents.addProperty("ADBE Vector Graphic - Fill");
        
        if (lyr.parent) sLayer.parent = lyr.parent;
        
        for (var t = 0; t<transA.length; t++) {
            var inProp = inTrans(transA[t]);
            var outProp = outTrans(transA[t]);
            if (inProp.numKeys) {
        ...
        sLayer.moveBefore(lyr);
        sLayer.name = lyr.name;
        lyr.remove();
    }
    // convert all selected layers to circle shapes
    function layersToCircles () {
        var comp = app.project.activeItem;
        if (comp != null && comp instanceof CompItem) {
            ...
            for (var i = 0; i < layerAry.length; i++) {
                makeCircleShape(layerAry[i]);
            }
            
            app.endUndoGroup();
            return alert("Conversion complete.");
    ...
    function buildUI(thisObj) {
        ...
        var ui = "group{orientation:'column', alignment:['fill','fill'],spacing:10,\
            ...
            mainGroup:Group{orientation:'column',alignment:['fill','top'],alignChildren:['fill','top'],\
                ...
                circleBtn: Button{preferredSize:[100,-1]},\
            }\
        }";
        pal.grp = pal.add(ui);
        ...
        pal.grp.mainGroup.mainText.text = "Convert selected layers to circle shape layers";
        
        // circle converter
        pal.grp.mainGroup.circleBtn.text = "Convert";
        pal.grp.mainGroup.circleBtn.onClick = layersToCircles;
